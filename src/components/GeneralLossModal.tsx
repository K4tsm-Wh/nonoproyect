...
import { registrarMermaFIFO, LOSS_REASONS, LossReason } from '@/services/lossService'
import { toast } from 'sonner'

interface GeneralLossModalProps {
  isOpen: boolean
  product: any
  stockTotal: number
  onClose: () => void
...
export default function GeneralLossModal({ 
  isOpen, 
  product, 
  stockTotal, 
  onClose, 
  onSuccess 
}: GeneralLossModalProps) {
  const [quantity, setQuantity] = useState<number>(1)
  const [reason, setReason] = useState<LossReason>(LOSS_REASONS.DAMAGED)
  ...
  useEffect(() => {
    if (isOpen) {
      setQuantity(1)
      setReason(LOSS_REASONS.DAMAGED)
      setNote('')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const result = await registrarMermaFIFO({
        sku: product.sku,
        quantity,
        reason,
      ...
      toast.success(`Merma registrada: ${result.totalDescontado.toFixed(2)}kg`, {
        description: `${product.nombre} • ${reason}`
      })
      onSuccess()
      onClose()
    } catch (err: any) {
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
        ...
        <div className="bg-gradient-to-r from-red-500 to-orange-500 p-4 text-white">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-red-100 text-sm">{product.nombre}</p>
            </div>
            <button 
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {/* Info del producto */}
          <div className="bg-orange-50 p-3 rounded-xl text-sm border border-orange-200">
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Stock total disponible:</span>
              <span className="font-bold text-orange-700">{stockTotal.toFixed(2)} kg</span>
            </div>
            <p className="text-xs text-orange-600 mt-1">
              ⚡ Se descontará automáticamente de los lotes más antiguos (FIFO)
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex justify-between mt-2">
              <button
                type="button"
                onClick={() => setQuantity(stockTotal)}
                className="text-xs text-red-600 hover:text-red-700 font-medium hover:underline"
              >
                Todo el stock ({stockTotal.toFixed(2)}kg)
              </button>
              <button
                type="button"
                onClick={() => setQuantity(Math.min(1, stockTotal))}
